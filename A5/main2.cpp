...
static void init()
{
	...
	int gridN = 50; // Number of subdivisions; can be adjusted
	float radius = 0.5f; // Sphere radius
	for (int i = 0; i <= gridN; ++i) {
		float theta = M_PI * (float)i / gridN; // Angle from the positive y-axis
		for (int j = 0; j <= gridN; ++j) {
			float phi = 2.0f * M_PI * (float)j / gridN; // Angle from the positive z-axis in the xz-plane
			...
			posBuf.push_back(y);
			posBuf.push_back(z);
			// Normal (for a sphere at the origin, the normal is the same as the position normalized)
			norBuf.push_back(x);
			norBuf.push_back(y);
			norBuf.push_back(z);
			// Texture coordinates (using spherical coordinates to map to Mercator projection)
			texBuf.push_back((float)j / gridN); // U coordinate
			texBuf.push_back(1.0f - (float)i / gridN); // V coordinate, flipped
		}
	}
...
static void render()
{
	...
	for (int i = 0; i < lights.size(); ++i) {
		...
		// Move to light position and scale for representation
		MV->translate(lights[i]->position);
		MV->scale(glm::vec3(0.1));  // Assuming you want the light spheres to be small

		// Set the model view matrix uniform
		glUniformMatrix4fv(useProg->getUniform("MV"), 1, GL_FALSE, glm::value_ptr(MV->topMatrix()));

		// Assume lights are white for now, but you can use the light's color
		...
		glEnableVertexAttribArray(0); // Assuming layout location 0 for positions
		glBindBuffer(GL_ARRAY_BUFFER, bufIDs["bPos"]);
		glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, (void*)0);

		glEnableVertexAttribArray(1); // Assuming layout location 1 for normals
		glBindBuffer(GL_ARRAY_BUFFER, bufIDs["bNor"]);
	...
	for (int i = 0; i < 10; ++i) {
		for (int j = 0; j < 10; ++j) {
			...
			else if (currentShape == ball) {
				MV->translate(position);
				MV->scale(glm::vec3(scale*0.2));
				MV->translate(glm::vec3(0, 2.0, 0));
			}
			glUniformMatrix4fv(useProg->getUniform("MV"), 1, GL_FALSE, glm::value_ptr(MV->topMatrix()));
			glUniformMatrix3fv(useProg->getUniform("invTransformMV"), 1, GL_FALSE, glm::value_ptr(glm::transpose(glm::inverse(glm::mat3(MV->topMatrix())))));
